...
(function (global) {
  ...
  function pigHead(o) {
    ...
    var parts = [
      ...
      /* le groin sort du milieu du visage, sous les yeux */
      '<ellipse cx="58" cy="-38" rx="23" ry="14" transform="rotate(-12 58 -38)" fill="%F%" %S%/>'
    ];
    var s = U(parts, skin, 9);
    // narines
    s += '<ellipse cx="66" cy="-44" rx="4.4" ry="5.6" fill="' + shade(skin, -0.42) + '"/>' +
      '<ellipse cx="72" cy="-35" rx="4.4" ry="5.6" fill="' + shade(skin, -0.42) + '"/>';
    // yeux, posés au-dessus du groin
    var ey = -63;
    s += '<circle cx="15" cy="' + ey + '" r="12.5" fill="#fff" stroke="' + INK + '" stroke-width="3.5"/>' +
      '<circle cx="40" cy="' + (ey + 4) + '" r="12.5" fill="#fff" stroke="' + INK + '" stroke-width="3.5"/>';
    if (mood === 'sleep') {
      s += line('M 7,' + ey + ' q 8,7 16,0', INK, 4) + line('M 32,' + (ey + 4) + ' q 8,7 16,0', INK, 4);
    } else if (mood === 'wow') {
      s += '<circle cx="17" cy="' + ey + '" r="6" fill="' + INK + '"/><circle cx="42" cy="' + (ey + 4) + '" r="6" fill="' + INK + '"/>';
    } else {
      s += '<circle cx="19" cy="' + (ey + 1) + '" r="5.4" fill="' + INK + '"/><circle cx="44" cy="' + (ey + 5) + '" r="5.4" fill="' + INK + '"/>';
    }
    // joue
    s += '<circle cx="-8" cy="-30" r="10" fill="' + shade(skin, -0.16) + '" opacity=".85"/>';
    // bouche, sous le groin
    if (mood === 'wow') s += '<ellipse cx="24" cy="-22" rx="8.5" ry="10" fill="#b8355c" stroke="' + INK + '" stroke-width="3.5"/>';
    else if (mood === 'sad') s += line('M 10,-16 q 14,-10 26,-8', INK, 5);
    else s += line('M 8,-24 q 16,15 30,-4', INK, 5);
    return s;
  }
  ...
  function pig(o) {
    ...
    if (o.glasses) {
      t += g('translate(0,-134)', '<circle cx="15" cy="-63" r="16" fill="none" stroke="' + INK + '" stroke-width="3.5"/>' +
        '<circle cx="40" cy="-59" r="16" fill="none" stroke="' + INK + '" stroke-width="3.5"/>' +
        line('M 30,-63 l 9,2', INK, 3.5));
    }
