...
(function (global) {
  ...
  var T = {
    fr: {
      ...
      labyConsigne: "Touche où tu veux aller. Livia marche jusqu'à l'étoile.",
      ...
      jLaby: 'Le labyrinthe', jLabySous: 'Touche, et Livia trouve le chemin',
    ...
    es: {
      ...
      labyConsigne: 'Toca adónde quieres ir. Livia camina hasta la estrella.',
      ...
      jLaby: 'El laberinto', jLabySous: 'Toca, y Livia encuentra el camino',
    ...
    en: {
      ...
      labyConsigne: 'Tap where you want to go. Livia walks to the star.',
      ...
      jLaby: 'The maze', jLabySous: 'Tap, and Livia finds the way',
  ...
  function jeuLabyrinthe(zone, n, api) {
    ...
    /* Une case de plus sur le chemin — ou une case de moins, si l'on revient
       sur ses pas : le trait se raccourcit alors au lieu de se dédoubler. */
    function pas(c) {
    ...
    /* ------------------------------------------------------------
       ...
       Suivre un couloir en gardant le doigt dessus demande une précision
       qu'on n'a pas à quatre ans : le doigt sort du tracé, saute une case,
       et le personnage ne bouge plus. On touche donc simplement un endroit
       du labyrinthe, et Livia s'y rend toute seule, en ligne droite, une
       case après l'autre — elle s'arrête au mur si le chemin est bouché.

       Le jeu reste entier : à chaque croisement, c'est toujours l'enfant qui
       choisit la direction. Ce qui disparaît, c'est l'exigence du geste.
       ------------------------------------------------------------ */
    var marche = null, fini = false, vise = -1;

    function versLaCase(cible) {
      if (fini || cible < 0) return;
      var d = chemin[chemin.length - 1];
      if (cible === d) return;
      var dx = (cible % cols) - (d % cols);
      var dy = Math.floor(cible / cols) - Math.floor(d / cols);
      /* une seule direction à la fois : la plus franche des deux */
      var ax = 0, ay = 0;
      if (Math.abs(dx) >= Math.abs(dy)) ax = dx > 0 ? 1 : (dx < 0 ? -1 : 0);
      else ay = dy > 0 ? 1 : (dy < 0 ? -1 : 0);
      if (!ax && !ay) return;
      var reste = ax ? Math.abs(dx) : Math.abs(dy);

      clearInterval(marche);
      marche = setInterval(function () {
        if (fini || reste-- <= 0) { clearInterval(marche); return; }
        var a = chemin[chemin.length - 1];
        var nx = (a % cols) + ax, ny = Math.floor(a / cols) + ay;
        if (nx < 0 || ny < 0 || nx >= cols || ny >= rangs) { clearInterval(marche); return; }
        var b = ny * cols + nx;
        if (!ouvert(a, b)) { clearInterval(marche); return; }   /* un mur : on s'arrête */
        pas(b);
      }, 135);
    }
    ...
    svg.addEventListener('pointerdown', function (e) {
      if (fini) return;
      trace = true; vise = -1;
      /* la capture du pointeur est un confort, pas une nécessité : si elle
         échoue, elle ne doit pas emporter le déplacement avec elle */
      try { svg.setPointerCapture && svg.setPointerCapture(e.pointerId); } catch (err) { }
      var c = caseSous(e);
      vise = c;
      versLaCase(c);
    });
    /* on peut aussi glisser le doigt : la marche se recalcule dès qu'il
       change de case, sans jamais exiger qu'il suive le couloir */
    svg.addEventListener('pointermove', function (e) {
      if (!trace || fini) return;
      e.preventDefault();
      var c = caseSous(e);
      if (c < 0 || c === vise) return;
      vise = c;
      versLaCase(c);
    });
