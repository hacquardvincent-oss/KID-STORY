...
(function (global) {
  ...
  var T = {
    fr: {
      ...
      labyConsigne: "Pose ton doigt et glisse : Livia te suit jusqu'à l'étoile.",
      ...
      jLaby: 'Le labyrinthe', jLabySous: 'Glisse ton doigt, Livia te suit',
    ...
    es: {
      ...
      labyConsigne: 'Pon el dedo y arrástralo: Livia te sigue hasta la estrella.',
      ...
      jLaby: 'El laberinto', jLabySous: 'Arrastra el dedo, Livia te sigue',
    ...
    en: {
      ...
      labyConsigne: 'Put your finger down and slide: Livia follows you to the star.',
      ...
      jLaby: 'The maze', jLabySous: 'Slide your finger, Livia follows',
  ...
  function jeuLabyrinthe(zone, n, api) {
    ...
    /* ------------------------------------------------------------
       Le déplacement : Livia colle au doigt.

       On ne vise pas une case, on ne relâche pas l'écran : on pose le doigt
       et on tire le personnage, qui suit dans les couloirs et se cogne aux
       murs. C'est le geste le plus direct qui soit, et le seul qui marche
       vraiment à quatre ans.

       Pour que Livia ne traverse jamais un mur, sa position n'est pas un
       point libre sur le plateau : elle vit SUR le réseau des couloirs. Elle
       est soit au centre d'une case, soit quelque part entre deux cases
       voisines et reliées. Le doigt ne fait que projeter sa position sur ce
       réseau — un mur devient alors infranchissable par construction, sans
       aucun calcul de collision.
       ------------------------------------------------------------ */
    var ici = depart;      /* la case où elle est */
    var versC = -1;        /* la case voisine vers laquelle elle avance, ou -1 */
    var avance = 0;        /* de 0 à 1, où elle en est entre les deux */

    function centre(c) {
      return { x: (c % cols) * P + P / 2, y: Math.floor(c / cols) * P + P / 2 };
    }
    function ouvert(a, b) {
      var c = cases[a];
    ...
    function position() {
      ...
      if (versC < 0) return a;
      var b = centre(versC);
      return { x: a.x + (b.x - a.x) * avance, y: a.y + (b.y - a.y) * avance };
    }

    function majTrait() {
      /* le trait suit Livia jusqu'à sa position exacte, pas seulement de
         centre en centre : il se dessine sous ses pieds pendant qu'elle glisse */
      var pts = chemin.map(function (c) {
        var k = centre(c); return k.x + ',' + k.y;
      });
      var p = position();
      pts.push(p.x.toFixed(1) + ',' + p.y.toFixed(1));
      trait.setAttribute('points', pts.join(' '));
      heros.setAttribute('transform',
        'translate(' + (p.x - P / 2).toFixed(1) + ',' + (p.y - P / 2).toFixed(1) + ')');
    }

    /* une case de plus sur le chemin — ou une de moins, si l'on revient sur
       ses pas : le trait se raccourcit alors au lieu de se dédoubler */
    function poser(c) {
      if (chemin.length > 1 && c === chemin[chemin.length - 2]) chemin.pop();
      else if (chemin[chemin.length - 1] !== c) chemin.push(c);
    }

    var fini = false;

    function suivre(fx, fy) {
      if (fini) return;
      var interdit = -1;   /* le couloir dont elle vient de ressortir */
      /* plusieurs cases peuvent être franchies dans un même mouvement si le
         doigt va vite : on avance tant qu'il reste devant */
      for (var pas = 0; pas < 16; pas++) {
        if (versC < 0) {
          var a = centre(ici);
          var dx = fx - a.x, dy = fy - a.y;
          if (Math.abs(dx) < 4 && Math.abs(dy) < 4) break;
          /* Le couloir le mieux aligné avec le doigt d'abord ; si un mur le
             ferme, celui de l'autre axe. C'est ce deuxième essai qui fait
             tourner les angles tout seuls : le doigt coupe au plus court, en
             diagonale, et Livia suit le couloir qui va dans ce sens-là. */
          var essais = [];
          if (Math.abs(dx) >= Math.abs(dy)) {
            essais.push([dx > 0 ? 1 : -1, 0]);
            if (Math.abs(dy) > 8) essais.push([0, dy > 0 ? 1 : -1]);
          } else {
            essais.push([0, dy > 0 ? 1 : -1]);
            if (Math.abs(dx) > 8) essais.push([dx > 0 ? 1 : -1, 0]);
          }
          var x = ici % cols, y = Math.floor(ici / cols), cible = -1;
          for (var k = 0; k < essais.length && cible < 0; k++) {
            var vx = essais[k][0], vy = essais[k][1];
            if (x + vx < 0 || x + vx >= cols || y + vy < 0 || y + vy >= rangs) continue;
            var c2 = ici + vx + vy * cols;
            if (c2 !== interdit && ouvert(ici, c2)) cible = c2;
          }
          if (cible < 0) break;                /* un mur : elle s'arrête ici */
          versC = cible; avance = 0;
        }
        /* dans un couloir : le doigt se projette sur le segment */
        var d = centre(ici), f = centre(versC);
        var ex = f.x - d.x, ey = f.y - d.y;
        var lg2 = ex * ex + ey * ey;
        var t2 = ((fx - d.x) * ex + (fy - d.y) * ey) / lg2;
        /* de combien le doigt déborde-t-il du couloir, sur le côté */
        var ecart = Math.abs((fx - d.x) * ey - (fy - d.y) * ex) / Math.sqrt(lg2);
        /* Plus le doigt part sur le côté, plus elle accepte de se ranger au
           bout le plus proche du couloir. C'est ce qui permet de couper les
           angles : sans cela, il faudrait poser le doigt pile au centre de
           chaque case avant de pouvoir tourner — et à quatre ans, personne
           ne vise aussi juste. */
        var marge = 0.02 + Math.min(ecart, 50) / 50 * 0.3;
        /* L'étoile est une grande case jaune : dès qu'elle y pose le pied,
           c'est gagné. Demander le centre exact, c'est refuser la victoire à
           un doigt qui s'arrête à un cheveu du but. */
        if (versC === arrivee && t2 > 0.55) t2 = 1;
        if (t2 >= 1 - marge) {                  /* la case voisine est atteinte */
          ici = versC; versC = -1; avance = 0; interdit = -1;
          poser(ici);
          if (ici === arrivee) {
            fini = true;
            majTrait();
            setTimeout(function () { api.reussi(); }, 550);
            return;
          }
          continue;
        }
        if (t2 <= marge) {                      /* elle ressort du couloir */
          interdit = versC; versC = -1; avance = 0;
          continue;
        }
        avance = t2;
        break;
    ...
    svg.addEventListener('pointerdown', function (e) {
      if (fini) return;
      tire = true;
      /* la capture garde le doigt lié au plateau même s'il sort du cadre */
    ...
    svg.addEventListener('pointerup', function () { tire = false; });
    svg.addEventListener('pointercancel', function () { tire = false; });
  }
